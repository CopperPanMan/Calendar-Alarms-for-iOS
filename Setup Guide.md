...
Install these Shortcuts in the Shortcuts app. You can put them in a new folder for better organization:
...
- **Calendar Alarms Actions** *(required for built-in actions and OpenHabits task alarms)*
  - Receives the action JSON generated by the Alarm Editor and also replaces the former standalone Task Alarm Resetter.
...
- Task alarms use the installed **Calendar Alarms Actions** shortcut to refresh OpenHabits state and reset the next task-loop alarm. The former standalone Task Alarm Resetter is no longer required.
